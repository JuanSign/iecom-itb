...
import { 
    ...
    RefreshCw, AlertCircle, Check, CreditCard
} from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
import { getNiceStageTwoUploadUrl, saveNiceStageTwoKey } from "@/actions/server/competition/nice";

// --- Types ---
type NiceDeliverableType = 'payment_proof' | 'proposal';
...
export function StageTwoSection({ 
    ...
    viewPaymentUrl?: string | null,
    viewProposalUrl?: string | null
}) {
    return (
        <Card className="border-l-4 border-l-violet-500 shadow-sm">
                ...
                <div className="flex justify-between items-start">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm font-medium text-muted-foreground">Main Selection</span>
                        </div>
                        <CardTitle className="text-2xl flex items-center gap-2">
                            Stage 2: Payment & Proposal
                        </CardTitle>
                        <CardDescription>
                            Complete the payment and submit your proposal to qualify for the next round.
                        </CardDescription>
                    </div>
                </div>
            </CardHeader>
            ...
            <CardContent className="pt-6 space-y-4">
                <NiceDeliverableItem 
                    teamId={teamId}
                    title="Payment Proof"
                    description="Upload receipt of registration fee payment."
                    type="payment_proof"
                    accept=".jpg,.jpeg,.png,.pdf"
                    currentValue={paymentProofLink}
                    viewUrl={viewPaymentUrl}
                />
                <NiceDeliverableItem 
                    teamId={teamId}
                    title="Proposal"
                    description="Submit your team's proposal (PDF only)."
                    type="proposal"
                    accept=".pdf"
                    currentValue={proposalLink}
                    viewUrl={viewProposalUrl}
                />
            </CardContent>
        </Card>
    );
...
// --- Deliverable Item Component (Adapted for NICE) ---
function NiceDeliverableItem({
    teamId, title, description, type, accept, currentValue, viewUrl
}: {
    teamId: string;
    title: string;
    description: string;
    type: NiceDeliverableType;
    accept: string;
    ...
    const handleFileUpload = async () => {
        ...
        try {
            const { signedUrl, key } = await getNiceStageTwoUploadUrl(teamId, type, selectedFile.name, selectedFile.type);
            ...
            await saveNiceStageTwoKey(teamId, type, key);
    ...
    return (
        <Card className={cn(
            ...
            hasSubmission && !isEditing 
                ? "bg-violet-50/50 dark:bg-violet-950/10 border-violet-200 dark:border-violet-800/50" 
                : "bg-card border-border",
            uploadSuccess && "ring-2 ring-violet-500/50"
        )}>
            <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="flex-1 flex items-start gap-4 w-full">
                    <div className={cn("p-2.5 rounded-lg border", hasSubmission ? "bg-violet-100 text-violet-600 border-violet-200" : "bg-muted text-muted-foreground")}>
                        {type === 'payment_proof' ? <CreditCard className="w-5 h-5" /> : <FileText className="w-5 h-5" />}
                    </div>
                    <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                            <h4 className="font-semibold">{title}</h4>
                            {hasSubmission && <Badge variant="outline" className="text-violet-600 bg-violet-50 border-violet-200 text-[10px] h-5 px-1.5"><CheckCircle2 className="w-3 h-3 mr-1"/> Submitted</Badge>}
                        </div>
                ...
                <div className="w-full md:w-[320px] shrink-0">
                    {(!hasSubmission || isEditing) ? (
                        <div className={cn("space-y-2", isEditing && "bg-muted/30 p-3 rounded-lg border border-dashed")}>
                            ...
                            {!selectedFile ? (
                                <Button variant="outline" className="w-full border-dashed text-muted-foreground hover:text-violet-600 hover:border-violet-300" onClick={() => fileInputRef.current?.click()} disabled={isUploading}>
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <Button onClick={handleFileUpload} disabled={isUploading} className="w-full bg-violet-600 hover:bg-violet-700">
                        ...
                        <div className="bg-background/60 border rounded-lg p-2.5 flex flex-col gap-2">
                            <div className="flex items-center gap-2 overflow-hidden">
                                <FileIcon className="w-4 h-4 text-violet-500 shrink-0"/>
